...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//select MAX3421E
	int ssret = XSpi_SetSlaveSelect(&SpiInstance, 0x01);

	//write reg + 2 via SPI
	uint8_t sendbuf[2];   //2 bytes will be sent: command and data - use a 2-element array
	uint8_t recbuf[2];   //initialization not necessary - data will be received
	sendbuf[0] = reg | 0x2;  //+2  sets R/W bit to 1
	...
	//read return code from SPI peripheral (see Xilinx examples)
	int retcode;
	retcode = XSpi_Transfer(&SpiInstance, sendbuf, recbuf, 2);   //sendbuf and recbuf treated as pointers to the array's first element

	//if return code != 0 print an error
	if (retcode != XST_SUCCESS) {xil_printf("Single host register write failed.\n");}

	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	 ssret = XSpi_SetSlaveSelect(&SpiInstance, 0x00);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//write reg + 2 via SPI
	uint8_t sendbuf[nbytes + 1];   //nbytes of data + 1 byte for command
	uint8_t recbuf[nbytes + 1];   //initialization not necessary - data will be received
	sendbuf[0] = reg | 0x2;  //+2 sets R/W bit to 1

	//write data[n] via SPI, where n goes from 0 to nbytes-1
	int i;
	for (i=0; i<nbytes; i++) {
	    sendbuf[i + 1] = data [i];   //using pointer-array duality
	}

	//read return code from SPI peripheral
	int retcode;
	retcode = XSpi_Transfer(&SpiInstance, sendbuf, NULL, nbytes+1);   //sendbuf and recbuf treated as pointers to the array's first element

	//if return code != 0 print an error
	if (retcode != XST_SUCCESS) {xil_printf("Multiple-byte write failed.\n");}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	uint8_t sendbuf[2];   //2 bytes will be sent: command and data - use a 2-element array
	uint8_t recbuf[2];   //initialization not necessary - data will be received
	sendbuf[0] = reg & 0xFD;  //to make sure R/W bit is 0
	sendbuf[1] = 0x00;  //dummy bytes
	...
	//if return code != 0 print an error
	if (retcode != XST_SUCCESS) {xil_printf("Single host register read failed.\n");}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//write reg via SPI
	uint8_t sendbuf[nbytes + 1];   //nbytes of data + command
	uint8_t recbuf[nbytes + 1];   //initialization not necessary - data will be received
	sendbuf[0] = reg & 0xFD;  //to make sure R/W bit is 0

	int i;
	for (i=1; i<=nbytes; i++) {
	   sendbuf[i] = 0x00;   //dummy bytes
	}
	...
	//read return code from SPI peripheral
	int retcode;
	retcode = XSpi_Transfer(&SpiInstance, sendbuf, recbuf, nbytes + 1);   //sendbuf and recbuf treated as pointers to the array's first element

	for (i=0; i<nbytes; i++) {
	    data[i] = recbuf[i+1];     //recbuf[0] skipped because is the byte returned in response to command byte = meaningless
	}

	//if return code != 0 print an error
	if (retcode != XST_SUCCESS) {xil_printf("Multiple-byte register read failed.\n");}

	//deselect MAX3421E (may not be necessary if you are using SPI perip22heral)
	 ssret = XSpi_SetSlaveSelect(&SpiInstance, 0x00);
